...
class PriorityQ:
	...
	func get_item_priority(arrayIdx):
		...
		if (arrayIdx + 1 > currentSize || arrayIdx < 0):
			...
			print(str(Time.get_ticks_msec()) + ": PRIORITY QUEUE (" + str(self.get_instance_id()) + "): get_item_priority(arrayIdx) out of bounds! arrayIdx = " + str(arrayIdx) + ", currentSize = " + str(currentSize))
			return false
